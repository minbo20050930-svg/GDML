...
class ISIC2018_dataset(Dataset):
    def __init__(self, dataset_folder='/ISIC2018_Task1_npy_all',
        ...
        if self.train_type in ['train', 'validation', 'test']:
            ...
            self.mask1 = [join(dataset_folder, 'body-origin', x.split('.')[0] + '_segmentation.npy') for x in
                          self.image_list]
            self.mask2 = [join(dataset_folder, 'detail-origin', x.split('.')[0] + '_segmentation.npy') for x in
                          self.image_list]
        else:
