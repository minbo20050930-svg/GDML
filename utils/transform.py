...
def ph2_transform_newdata(sample, train_type):
    image, label1, label2 = Image.fromarray(np.uint8(sample['image']), mode='RGB'), \
        Image.fromarray(np.uint8(sample['label1']), mode='L'), \
        Image.fromarray(np.uint8(sample['label2']), mode='L')
    label = sample['label']
    if train_type == 'train':
        # image, label = randomcrop(size=(224, 320))(image, label)
        image, label, label1, label2 = randomflip_rotate(image, label, label1, label2, p=0.5, degrees=30)
    else:
        image = image
        label = label
        label1 = label1
    ...
    label1 = ts.ToTensor()(label1)
    label2 = ts.ToTensor()(label2)

    return {'image': image, 'label': label, 'label1': label1, 'label2': label2}
...
def ISIC2018_transform_newdata(sample, train_type):
    image, label, label1 , label2 = Image.fromarray(np.uint8(sample['image']), mode='RGB'),\
                                    Image.fromarray(np.uint8(sample['label']), mode='L'),\
                                    Image.fromarray(np.uint8(sample['label1']),mode='L'),\
                                    Image.fromarray(np.uint8(sample['label2']),mode='L')
...
def randomflip_rotate(img, lab,lab1, lab2, p=0.5, degrees=0):
    if random.random() < p:
        img = TF.hflip(img)
        lab = TF.hflip(lab)
        lab1 = TF.hflip(lab1)
    ...
    lab1 = TF.rotate(lab1, angle)
    lab2 = TF.rotate(lab2, angle)

    return img, lab, lab1, lab2
